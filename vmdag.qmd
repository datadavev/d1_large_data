...
Initial state of VMDAG. The root of the first version is `dbc15` referenced by `PID_1`.
:::
...
State of VMDAG after adding a file to folder B. The root of the new version is `ed44a` which is referenced by `PID_2`. The previous version is retained, with only changes being recorded.
:::

Assuming the bytes of the original content are preserved, the entire state of a version of the folder hierarchy can be retrieved given the hash of the root for the desired version, i.e. by resolving `PID_1` or `PID_2`. 

This approach is very efficient for recording changes to the tree, since only the changed folder and its parents need to be recorded. It is especially effective in an object store like `hashstore` which uses the content hashes to locate content.
...
: Recursively compute the list of Merkle tree entries that represent the folder hierarchy. This will be a depth first traversal of the folder hierarchy, computing the hash for each file and folder, and returning a list of entries with the format described below. The hash for each folder is computed from the list of entries for that folder, sorted by `ftype` and `name`. The hash for each file is computed from the content of the file. The root folder will be the last entry in the list. 
...
`ftype` is a constant that indicates the type of the entry. For files, `ftype` = 1 and `hash` is the computed hash of the content. For folders, `ftype` = 0 and `hash` is the hash value computed from the list of entries in the folder, sorted by `ftype` and `name`. The `folder_hash` value is the hash of the containing folder, which is computed after the entries for the folder are known.
...
Internal composition of a hashstore.
:::
...
When generating the individual `Folder` files for option 1, it is critical that the serialization of the folder objects is consistent, since the hash of the file itself is used as the `folder_hash` value. This means that the entries for a folder must be sorted by `ftype` and `name` before being serialized, and that the serialization format must be deterministic (e.g. JSON with sorted keys).
...
This requires that a client be able to send the individual `Folder` entries to the server if Option 1 is used for VMDAG storage, or  the entire VMDAG if Option 2 is used for VMDAG storage.
...
2. File tagging. Each file and folder can be assigned a persistent unique identifier (like a SID in DataONE) as an extended attribute. The xattr will remain attached to the file through mutations and moves, and could thus provide the basis for computing the change operations for a new MDAG version. Extended attributes are available in Linux and MacOS, though is more challenging to implement on Windows.
...
Recommendation is to implement option 1 and extend to option 2 when needed[^opt12].

[^opt12]: Option 2 is a little more complex to implement, but would be more efficient for large hierarchies. However, it requires that the client environment support extended attributes which may be a barrier for some users. 
...
For this to work with `Folder` storage option 1 in hashstore, the hash algorithm must be identical between the client and the server, and the client must be able to send the individual `Folder` entries to the server.